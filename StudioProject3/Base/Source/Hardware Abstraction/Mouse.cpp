...
int CMouse::Read(const float deltaTime)
{
	...
	Zoom_InOut(deltaTime, true,
		MouseController::GetInstance()->GetMouseScrollStatus(MouseController::SCROLL_TYPE::SCROLL_TYPE_YOFFSET));
...
void CMouse::SetTroopsDestination(double x, double y, int w, int h)
{
	//// Get normalised device space coords
	//float xPos = (2.0f * x) / 800.f - 1.0f;
	//float yPos = 1.0f - (2.0f * y) / 600.f;
	//float zPos = 1.0f;
	//Vector3 ray_nds = Vector3(xPos, yPos, zPos);

	//// Get Homogeneous Clip Space Coords
	//Vector3 ray_clip = Vector3(ray_nds.x, ray_nds.y, -1.f);

	//// Get Eye (Camera) Coords
	//Mtx44 perspective;
	//perspective.SetToPerspective(45.0f, 4.0f / 3.0f, 0.1f, 10000.0f);
	//Vector3 ray_eye = perspective.GetInverse() * ray_clip;
	//ray_eye = Vector3(ray_eye.x, ray_eye.y, -1.0f); //set z to be forward

	//// Get World Coords
	//Vector3 ray_wor = CPlayerInfo::GetInstance()->GetCamera()->GetViewMatrix().GetInverse() * ray_eye;
	//ray_wor = ray_wor.Normalize();

	////Testing of ray
	//float distanceFromRay = -(CPlayerInfo::GetInstance()->GetPos().Dot(Vector3(0, 1, 0) + 0) / ray_wor.Dot(Vector3(0, 1, 0))) - 20.f;
	//test = Vector3(CPlayerInfo::GetInstance()->GetPos().x + ray_wor.x * distanceFromRay, 10.f, CPlayerInfo::GetInstance()->GetPos().z + ray_wor.z * distanceFromRay);
	//
