...
void CSoundEngine::PlayASound(const std::string& _soundIndex, bool loop)
{
	...
	{
		// Play a sound
		theSoundEngine->play2D(aSound.c_str(), loop, false);
		//theSoundEngine->play3D(aSound.c_str());
	}
...
//void CSoundEngine::setVolume(int volume);
//void CSoundEngine::increaseVolume(int volume);
//void CSoundEngine::decreaseVolume(int volume);